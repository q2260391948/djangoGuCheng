...
def register(request):
    ...
    if request.GET:
        ...
        password_question = request.GET['password_question']
        password_answer = request.GET['password_answer']
        print(user, password, nickname,password_question,password_answer)
        ok = 1
        try:
            old_user = models.User.objects.get(user_id=user)
            ok = 0
        except Exception as e:
            user = models.User(user_id=user, user_nick=nickname, user_password=password,password_question=password_question,password_answer=password_answer)
            user.save()
...
def page2_template(request):
    return render(request, "page2_template.html")

# def get_user_question(request):
# def get_question(request):
#     question = User_Question.objects.all()
#     return render(request, 'page2_template.html', {'question': question})

def get_question(request):

    my_input_userid = request.POST.get('my_input_userid', '')
    # answer = request.POST.get('answer', '')
    question = User_Question.objects.get(user_id=my_input_userid).question
    nick_name = models.User.objects.get(user_id=my_input_userid).user_nick
    print(question)
    print(nick_name)
    print(my_input_userid)
    # print(answer)
    return render(request, 'input_username.html', {'my_input_userid': my_input_userid,'nick_name':nick_name,'question':question})

def get_question3(request):
    answer = request.POST.get('answer', '')
    user_id =request.POST.get('user_id','')
    your_answer = models.User_Question.objects.get(user_id=user_id).answer
    if(your_answer==answer):
        your_password = models.User.objects.get(user_id=user_id).user_password
        print(your_password)
        return render(request, 'test2.html', {'answer': answer, 'user_id': user_id, "your_password": your_password})
    else:
        str = "回答问题错！"
        print(user_id)
        print("--------------")
        print(answer)
        return render(request, 'wrong_answer.html', {'answer': answer, 'user_id': user_id})



def my_view(request):
    answer = User_Question.objects.all()[0].answer

    # print('用户名是',my_input_userid)
    if request.method == 'POST':
        # 获取输入框的数据
        my_input = request.POST.get('my_input', '')
        nick_name = models.User.objects.get(user_id=my_input).user_nick
        print(nick_name)
        if my_input==answer:
            print("bingo")
            # question = User_Question.objects.all()
        ...
        # 处理数据...
        print(my_input)
        # return HttpResponse(f"Received: {my_input}")
        return render(request, 'test2.html', {'nick_name': nick_name})
